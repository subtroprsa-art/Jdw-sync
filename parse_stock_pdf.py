...
def parse_comm_field(comm_str):
    """
    Parse commodity field e.g. AVOS,TR040,AF,1,*,14,*
    Format: COMMODITY, PACK_CODE, VARIETY, GRADE, sub, SIZE, *
    Index:  0          1          2        3      4    5     6

    Examples:
      AVOS,TR040,AF,1,*,14,*   -> size=14
      AVOS,BG150,AH,2,M,*,*   -> size=M  (bag packs use M/L/S)
      KIWI,PTB005,*,1,*,20,*  -> size=20
      LEMS,CTT150,*,1,*,56,*  -> size=56
      NAAR,CO100,LR,1,1X,*,*  -> size=1X (grade size)
    """
    ...
    # Size is at index 5 — but if index 5 is * then fall back to index 4
    raw_size5 = parts[5] if len(parts) > 5 else '*'
    raw_size4 = parts[4] if len(parts) > 4 else '*'

    if raw_size5 and raw_size5 != '*':
        size = raw_size5
    elif raw_size4 and raw_size4 != '*':
        size = raw_size4
    else:
        size = '*'
    ...
    if pack    == '*': pack    = ''
    if variety == '*': variety = '*'
    if not grade or not grade[0].isdigit(): grade = '1'
...
def parse_layout_line(line_text):
    ...
    # Must contain a GRN (8-digit number starting with 15)
    grn_match = re.search(r'\b(15\d{6})\b', t)
    if not grn_match:
        return None
